...
public interface ClientRepository extends JpaRepository<Client, Long> {
    ...
    @Query("SELECT c FROM Client c JOIN ClientAccess ca ON ca.clientId = c.id WHERE ca.authClientId = :authClientId AND (LOWER(c.firstName) LIKE LOWER(CONCAT('%', :query, '%')) OR LOWER(c.lastName) LIKE LOWER(CONCAT('%', :query, '%'))) ORDER BY c.id ASC")
